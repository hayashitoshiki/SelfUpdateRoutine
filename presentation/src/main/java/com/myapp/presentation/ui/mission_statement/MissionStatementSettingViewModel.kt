...
class MissionStatementSettingViewModel(
    ...
    init {
        ...
        missionStatement?.also {
            _funeralList.value = it.funeralList.map { funeral ->
            ...
            _constitutionList.value = it.constitutionList.map { constitution ->
    ...
    fun onClickConfirmButton() {
        val funeral = funeralList.value?.map {
            it.second
        } ?: run {
        ...
        val constitutionList = constitutionList.value?.map {
            it.second
        } ?: run {
    ...
    private fun changeButton() {
        val funeral = _funeralList.value?.map { it.second } ?: return
        val purposeLife = purposeLife.value ?: return
        val constitutionList = _constitutionList.value?.map { it.second } ?: return
        _isEnableConfirmButton.value = !(funeral.size == 1 && funeral.first()
            .isEmpty() && purposeLife.isEmpty() && constitutionList.size == 1 && constitutionList.first()
            .isEmpty())
    }
