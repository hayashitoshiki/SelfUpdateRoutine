 ...
 */
@Composable
fun WeatherReasonScreen(
    navController: NavHostController,
    viewModel: WeatherReasonViewModel
) {

    // label
    val section = LocalContext.current.getString(R.string.section2_2)
    val title = LocalContext.current.getString(R.string.title_item_reason)
...
@Composable
fun PreviewWeatherReasonScreen() {
    val navController = rememberNavController()
    val viewModel: WeatherReasonViewModel = hiltViewModel()

    Scaffold(backgroundColor = Color(0xfff5f5f5)) {
        WeatherReasonScreen(navController, viewModel)
    }
}