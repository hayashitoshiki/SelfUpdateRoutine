 ...
 * 振り返り_教訓画面 画面ロジック仕様
 */
class FfsLearnViewModelTest {

    private val state = DiaryBaseContract.State()
    private lateinit var ffsFactViewModel: FfsLearnViewModel
    ...
    fun setUp() {
        ...
        ffsFactViewModel = FfsLearnViewModel()
     ...
     * 　　・何もイベントが発生しないこと
     * ・共通処理イベント
     * 　　・入力した文字を引数に教訓入力アクションが流れること
     */
    @ExperimentalCoroutinesApi
    @Test
    fun changeLearnByInput() = testScope.runBlockingTest {

        // 期待結果
        val value = "fact"
        val expectationsState = state.copy(inputText = value, isButtonEnable = true)
        val expectationsEffect = null
        val expectationsAction = DiaryDispatcherContract.Action.ChangeLearn(value)
    ...
    @ExperimentalCoroutinesApi
    @Test
    fun changeLearnByAllDelete() = testScope.runBlockingTest {
